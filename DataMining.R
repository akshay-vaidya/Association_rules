#Â Data Mining Code (Association Rules)
#
#
# This set of notes shows how we can use association rule mining to analyze voting data. 
#Â The example uses a famous data from the US Congress where there sixteen key votes from 1984 are considered. 
#Â There are 435 members of congress and they are either Democrat or Republican.
# We can investigate if there are structures in the voting behaviour of the congress members
#Â Further details on the data are available here: http://archive.ics.uci.edu/ml/datasets/Congressional+Voting+Records

#Â Turn on the arules package

library(arules)

#Â Read in data from the web
...
#Â Note that the ?'s are people being absent from the vote.
# We recode the ?'s as n's

dat[dat=="?"] <- "n"

#Â I will remove "party" from the data so that we only analyze votes.
...
#Â Getting the data into the transaction format that arules uses. We look at the data again.
...
#Â Getting the data into the format that arules uses.
...
#Â We get many more rules so we may wish to consider changing the parameter settings. 
